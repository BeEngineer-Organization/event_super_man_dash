...
class Boy:
    ...
    def update(self, scroll_x, enemies, game_scene, coins, itemblocks):
        ...
        if self.jump_status == 0:
            pass
        elif self.jump_status == 1:
            self.y = self.initial_position_y + self.v_y_0 * self.time + 0.5 * GRAVITY * self.time ** 2

            # v = v0 + at で現在の速度を取得する
            # self.v_y が self.v_y_0 + GRAVITY * self.time に置き換わるイメージ
            if self.v_y_0 + GRAVITY * self.time < 0:
                jumping_distance = 0
                while jumping_distance >= self.v_y_0 + GRAVITY * self.time:
                    jumping_coll_flags = detect_collision(self.x, self.y + jumping_distance)
                    if jumping_coll_flags[0] or jumping_coll_flags[1] or jumping_coll_flags[4]:
                        self.y += jumping_distance
                        self.time = 0
        ...
        if pyxel.btnp(pyxel.KEY_SPACE) and (coll_flags[2] or coll_flags[3]):
            ...
            self.v_y_0 = -10
            self.jump_status = 1
            self.initial_position_y = self.y
            self.time = 0
        ...
        for itemblock in itemblocks:
            if (
                ...
                if self.prev_y >= itemblock.y + itemblock.h:
                    ...
                    coins.append(
                        Coin(itemblock.x, itemblock.y - 16)
                    )
